...
("whose call?", "spans domain and UI?") and to write the failing tests.
...
Playwright slices change only `*.e2e.ts`, while Vitest slices change none.
...
effort. The driver resolves this file relative to its own source, not the shell's
current directory, and validates it before any GitHub write, worktree creation
or agent turn. Both roles and all three keys are required; unknown roles or keys
are errors. AI Army allows a model flag to be omitted, but this driver requires
an explicit non-empty model so a run never inherits a changing backend default.
...
Corrective mechanic turns keep the mechanic entry selected at startup and reuse
the same AI Army team/session, branch and worktree; editing `agents.yaml` during
a run does not change an in-flight slice.
...
The current block 1 schema and narration use `orchestrator` as the driver's
owner label; it means the Python driver, not an external supervising session.
...
- **Stopped or failed:** read the comment the run left on the story. It says
  ...
  preserved in place for audit; inspect it before using the destructive
