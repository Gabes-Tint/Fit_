...
describe('nextUndoneSet', () => {
	it('returns the first undone set of the first exercise that has one', async () => {
		...
		expect((await import('./workout')).nextUndoneSet(w)).toEqual({ exerciseIndex: 0, setIndex: 2 });
	});
	...
	it('returns null when every set is done', async () => {
		...
		expect((await import('./workout')).nextUndoneSet(w)).toBeNull();
	});
	...
	it('finds the first undone set in routine order', async () => {
		...
		expect((await import('./workout')).nextUndoneSet(w)).toEqual({ exerciseIndex: 1, setIndex: 0 });
	});
...
describe('setCounts', () => {
	it('returns done and total sets counted across all exercises', async () => {
		...
		expect((await import('./workout')).setCounts(w)).toEqual({ done: 7, total: 9 });
	});

	it('counts nothing for an empty workout', async () => {
		const w = workout([]);
		expect((await import('./workout')).setCounts(w)).toEqual({ done: 0, total: 0 });
	});
	...
	it('counts zero done sets when nothing is ticked', async () => {
		...
		expect((await import('./workout')).setCounts(w)).toEqual({ done: 0, total: 5 });
	});
	...
	it('counts all sets done when every one is ticked', async () => {
		...
		expect((await import('./workout')).setCounts(w)).toEqual({ done: 5, total: 5 });
	});
});
